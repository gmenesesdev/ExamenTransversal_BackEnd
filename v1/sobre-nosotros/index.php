        ...
        switch ($_metodo) {
            ...
            case 'POST':
                if ($_header == $_token_post) {
                    ...
                    if ($rs) {
                        ...
                        echo json_encode(["Error" => "Error al crear seccion sobre nosotros"]);
            ...
            case 'PUT':
                if ($_header == $_token_put) {
                    ...
                    $objeto = json_decode(file_get_contents("php://input", true));
                    switch (true) {
                        case property_exists($objeto, 'logo_color') && $objeto->logo_color && property_exists($objeto, 'descripcion') && $objeto->descripcion:
                            $rs = $control->putAll($objeto->id, $objeto->logo_color, $objeto->descripcion, $objeto->valor);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'logo_color') && $objeto->logo_color:
                            $rs = $control->putLogoColorByID($objeto->logo_color, $objeto->id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
