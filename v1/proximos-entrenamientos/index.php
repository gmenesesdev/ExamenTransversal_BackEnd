        ...
        switch ($_metodo) {
            ...
            case 'POST':
                if ($_header == $_token_post) {
                    ...
                    $rs = $control->postNewEntrenamiento($objeto);
                    if ($rs) {
                        ...
                        echo json_encode(["Error" => "Error al crear nuevo entrenamiento"]);
            ...
            case 'PUT':
                if ($_header == $_token_put) {
                    ...
                    $objeto = json_decode(file_get_contents("php://input", true));
                    switch (true) {
                        case property_exists($objeto, 'fecha') && $objeto->fecha && property_exists($objeto, 'hora') && $objeto->hora && property_exists($objeto, 'entrenamiento_lugar_id') && $objeto->entrenamiento_lugar_id:
                            $rs = $control->putAll($objeto->id, $objeto->fecha, $objeto->hora, $objeto->entrenamiento_lugar_id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'fecha') && $objeto->fecha:
                            $rs = $control->putFechaByID($objeto->id, $objeto->fecha);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'hora') && $objeto->hora:
                            $rs = $control->putHoraByID($objeto->id, $objeto->hora);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'entrenamiento_lugar_id') && $objeto->entrenamiento_lugar_id:
                            $rs = $control->putLugarByID($objeto->id, $objeto->entrenamiento_lugar_id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        default:
