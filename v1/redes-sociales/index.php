        ...
        switch ($_metodo) {
            ...
            case 'POST':
                if ($_header == $_token_post) {
                    ...
                    if ($rs) {
                        ...
                        echo json_encode(["Error" => "Error al crear redes sociales"]);
            ...
            case 'PUT':
                if ($_header == $_token_put) {
                    ...
                    $objeto = json_decode(file_get_contents("php://input", true));
                    switch (true) {
                        case property_exists($objeto, 'nombre') && $objeto->nombre && property_exists($objeto, 'icono') && $objeto->icono && property_exists($objeto, 'valor') && $objeto->valor:
                            $rs = $control->putAll($objeto->id, $objeto->nombre, $objeto->icono, $objeto->valor);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'nombre') && $objeto->nombre:
                            $rs = $control->putNombreByID($objeto->nombre, $objeto->id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'icono') && $objeto->icono:
                            $rs = $control->putIconoByID($objeto->icono, $objeto->id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        case property_exists($objeto, 'valor') && $objeto->valor:
                            $rs = $control->putValorByID($objeto->valor, $objeto->id);
                            http_response_code(200);
                            echo json_encode(["data" => $rs]);
                            break;
                        default:
